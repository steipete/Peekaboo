...
| VIS-003 | Watch capture channel (`showWatchCapture`) is a no-op, so users get no feedback during `peekaboo watch`. | 🟩 Fixed | Dedicated watch HUD now has its own settings toggle plus a timeline/tick indicator so watch sessions no longer look like screenshot flashes (VisualizerCoordinator + WatchCaptureHUDView.swift). |
