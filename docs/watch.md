...
- `contact.png`: auto grid (max 6 columns; rows inferred). Include contact metadata (cols/rows/thumb size, sampled frame indexes). For large frame counts, sample evenly across the sequence instead of placing every frame.
