...
| --- | --- | --- |
| `click` | Activate an application menu item via `--item` (single-level) or `--path "File > Export > PDF"`. | `--app <name|bundle|PID:1234>` (required), optional `--pid`, plus all focus flags. Paths are normalized automatically if you accidentally pass a `'>'` string to `--item`. |
...
| `list` | Dump the menu tree for a specific app (optionally showing disabled items). | Same targeting flags as `click`, plus `--include-disabled`. |
...
- `click` resolves apps via `ApplicationResolvablePositional`, so bundle IDs, friendly names, and `PID:####` tokens all work. It also calls `ensureFocusIgnoringMissingWindows`, which tolerates apps that keep a menu bar without a visible window (e.g., Finder when all windows are closed).
