...
- `peekaboo click --on/--id`, `move --on/--id`, and `drag --from/--to` now refresh the implicit observation snapshot once when a cached element ID is missing.
...
- done: refresh implicit observation snapshot once for `click --on/--id`, `move --on/--id`, and `drag --from/--to` when a cached element ID is missing;
