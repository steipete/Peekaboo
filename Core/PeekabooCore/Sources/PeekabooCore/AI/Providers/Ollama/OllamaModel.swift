...
public final class OllamaModel: ModelInterface {
    ...
    private var supportsTools: Bool {
        ...
        if Self.modelsWithToolSupport.contains(self.modelName) {
            return true
        }

        // Check if model is in the known unsupported list
        if Self.modelsWithoutToolSupport.contains(self.modelName) {
            return false
        }
        ...
        for supportedModel in Self.modelsWithToolSupport {
            ...
            if self.modelName.hasPrefix(baseModel) {
                return true
            }
        }
    ...
    private func getToolSupportError() -> String {
        if Self.modelsWithoutToolSupport.contains(self.modelName) {
            "Model '\(self.modelName)' does not support tool calling. Vision models like llava and bakllava cannot use tools. Please use llama3.3 or another model with tool support."
        } else {
            "Model '\(self.modelName)' may not support tool calling. Recommended models: llama3.3, llama3.2, mistral-nemo, firefunction-v2, or command-r-plus."
        }
    }
    ...
    public func getResponse(request: ModelRequest) async throws -> ModelResponse {
        ...
        aiDebugPrint("[OllamaModel] Sending request to \(url) for model \(self.modelName)")
    ...
    public func getStreamedResponse(request: ModelRequest) async throws -> AsyncThrowingStream<StreamEvent, Error> {
        aiDebugPrint("[OllamaModel] getStreamedResponse called for model: \(self.modelName)")
        ...
        return AsyncThrowingStream { continuation in
            ...
            Task.detached { [self, request, capturedSession, debugModelName, capturedSelf] in
                do {
                    ...
                    aiDebugPrint(
                        "[OllamaModel] Got response, status: \((response as? HTTPURLResponse)?.statusCode ?? -1)")
                                   ...
                                   let chunk = try? JSONCoding.decoder.decode(OllamaStreamChunk.self, from: data)
                                {
                                    ...
                                    if let content = chunk.message?.content, !content.isEmpty {
                                        ...
                                        if trimmedContent.hasPrefix("\",") || trimmedContent.hasPrefix("\"}") {
                                            ...
                                            if let data = content.data(using: .utf8),
                                               let toolCall = try? JSONCoding.decoder.decode(
                                                   TextBasedToolCall.self,
                                                   from: data),
                                               toolCall.type == "function"
                                            {
                                                // Convert parameters to JSON string
                                                let paramsString = self
                                                    .convertToolParameterDictToJSON(toolCall.parameters)
                                                ...
                                                continuation
                                                    .yield(StreamEvent.textDelta(StreamTextDelta(delta: content)))
                                            }
                                        } else {
                                    ...
                                    if let toolCalls = chunk.message?.toolCalls, !toolCalls.isEmpty {
                                        ...
                                        for toolCall in toolCalls {
                                            aiDebugPrint(
                                                "[OllamaModel] Tool call: \(toolCall.function.name) with args: \(toolCall.function.arguments)")
                                            ...
                                            aiDebugPrint(
                                                "[OllamaModel] Yielding tool call event: \(toolCall.function.name)")
            ...
            case let .user(_, content):
                ...
                case let .image(imageContent):
                    ...
                    if let base64 = imageContent.base64 {
                        ...
                        return OllamaMessage(
                            role: "user",
                            content: "[Image URL not supported by Ollama]",
                            images: nil,
                            toolCalls: nil)
                    }
                ...
                case let .audio(audioContent):
                    ...
                    if let transcript = audioContent.transcript {
                        ...
                        return OllamaMessage(
                            role: "user",
                            content: "[Audio requires transcription for Ollama]",
                            images: nil,
                            toolCalls: nil)
                    }
                }
    ...
    init(from decoder: Decoder) throws {
        ...
        if container.decodeNil() {
            ...
            throw DecodingError.dataCorruptedError(
                in: container,
                debugDescription: "Cannot decode tool parameter value")
        }
    }
