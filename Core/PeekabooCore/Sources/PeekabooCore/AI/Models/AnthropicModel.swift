...
public final class AnthropicModel: ModelInterface {
    ...
    public func getResponse(request: ModelRequest) async throws -> ModelResponse {
        ...
        if httpResponse.statusCode != 200 {
            ...
            if let errorResponse = try? JSONDecoder().decode(AnthropicErrorResponse.self, from: data) {
                throw handleAnthropicError(errorResponse, statusCode: httpResponse.statusCode)
            }
                ...
                do {
                    ...
                    if httpResponse.statusCode != 200 {
                        ...
                        if let errorResponse = try? JSONDecoder().decode(AnthropicErrorResponse.self, from: errorData) {
                            continuation.finish(throwing: self.handleAnthropicError(errorResponse, statusCode: httpResponse.statusCode))
                        } else {
                        ...
                        if line.hasPrefix("data: ") {
                            ...
                            if let eventData = data.data(using: .utf8) {
                                do {
                                    ...
                                    aiDebugPrint("DEBUG: Failed to parse event: \(error)")
    ...
    private func convertUserMessage(_ message: UserMessageItem) throws -> AnthropicMessage {
        ...
        case .image(let imageContent):
            ...
            if let base64 = imageContent.base64 {
                ...
                throw ModelError.invalidConfiguration("Image URLs not supported - please provide base64 data")
            }
        ...
        case .file:
            throw ModelError.invalidConfiguration("File content not supported in Anthropic API")
        }
    }
    ...
    private func handleAnthropicError(_ errorResponse: AnthropicErrorResponse, statusCode: Int) -> Error {
        ...
        case 400 where error.type == "invalid_request_error":
            if error.message.contains("context length") || error.message.contains("token") {
                return ModelError.contextLengthExceeded
            }
            return ModelError.invalidConfiguration(error.message)
        default:
