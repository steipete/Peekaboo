...
public final class GrokModel: ModelInterface {
    ...
    public func getResponse(request: ModelRequest) async throws -> ModelResponse {
        ...
        if httpResponse.statusCode != 200 {
            ...
            if let errorResponse = try? JSONDecoder().decode(GrokErrorResponse.self, from: data) {
                throw self.handleGrokError(errorResponse, statusCode: httpResponse.statusCode)
            }
            ...
            Task {
                do {
                    ...
                    if httpResponse.statusCode != 200 {
                        ...
                        if let errorResponse = try? JSONDecoder().decode(GrokErrorResponse.self, from: errorData) {
                            continuation.finish(throwing: self.handleGrokError(errorResponse, statusCode: httpResponse.statusCode))
                        } else {
                    ...
                    for try await line in bytes.lines {
                        ...
                        if line.hasPrefix("data: ") {
                            ...
                            if let chunkData = data.data(using: .utf8),
                               let chunk = try? JSONDecoder().decode(GrokChatCompletionChunk.self, from: chunkData) {
    ...
    private func handleGrokError(_ errorResponse: GrokErrorResponse, statusCode: Int) -> Error {
