...
public enum MouseLocationUtilities {
    ...
    public static func findApplicationAtMouseLocation() -> NSRunningApplication? {
                ...
                for window in windows {
                    if let frame = window.frame(),
                       frame.contains(mouseLocation) {
                        logger.debug("Mouse is over frontmost app: \(frontApp.localizedName ?? "unknown")")
                        return frontApp
                    }
                }
            }
        ...
        for app in visibleApps {
            ...
            if let windows = appElement.windows() {
                for window in windows {
                    if let frame = window.frame(),
                       frame.contains(mouseLocation) {
                        logger.debug("Found app at mouse location: \(app.localizedName ?? app.bundleIdentifier!)")
                        return app
                    }
                }
            }
        ...
        logger.debug("No app at mouse location, using frontmost: \(frontmostApp?.localizedName ?? "unknown")")
        return frontmostApp
    }
}