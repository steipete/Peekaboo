...
public final class ElementLayoutEngine {
    ...
    public func calculateLabelPosition(
        ...
        if case let .circle(diameter, position) = indicatorStyle {
            let indicatorPos = calculateIndicatorPosition(for: bounds, style: indicatorStyle)
    ...
    public func expandedBounds(
        ...
        return bounds.insetBy(dx: -expansion, dy: -expansion)
    }
    ...
    public func elementsOverlap(_ element1: VisualizableElement, _ element2: VisualizableElement) -> Bool {
        return element1.bounds.intersects(element2.bounds)
    }
    
    /// Find overlapping elements in a collection
    public func findOverlappingElements(in elements: [VisualizableElement]) -> [(VisualizableElement, VisualizableElement)] {
    ...
    public func optimizeLabelPositions(
        ...
        for element in sortedElements {
            var bestPosition = calculateLabelPosition(
                for: element.bounds,
            ...
            if occupiedRects.contains(where: { $0.intersects(labelRect) }) {
                let alternatives = generateAlternativePositions(
                    for: element.bounds,
                    containerSize: containerSize,
                    labelSize: labelSize
                )
