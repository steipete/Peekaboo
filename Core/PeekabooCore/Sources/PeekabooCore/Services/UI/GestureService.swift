...
public final class GestureService: Sendable {
    ...
    public func swipe(from: CGPoint, to: CGPoint, duration: Int, steps: Int) async throws {
        logger.debug("Swipe requested - from: (\(from.x), \(from.y)) to: (\(to.x), \(to.y)), duration: \(duration)ms, steps: \(steps)")
    ...
    public func drag(
        ...
        logger.debug("Drag requested - from: (\(from.x), \(from.y)) to: (\(to.x), \(to.y)), duration: \(duration)ms, modifiers: \(modifiers ?? "none")")
    ...
    public func moveMouse(to: CGPoint, duration: Int, steps: Int) async throws {
        logger.debug("Mouse move requested - to: (\(to.x), \(to.y)), duration: \(duration)ms, steps: \(steps)")
