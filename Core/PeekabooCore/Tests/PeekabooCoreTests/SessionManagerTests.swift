...
struct SessionManagerTests {
    ...
    func testCreateAndRetrieveSession() async throws {
        ...
        // Clean up
        try await sessionManager.cleanSession(sessionId: sessionId)
    }
    
    @Test("Store and retrieve detection result")
    func testStoreAndRetrieveDetectionResult() async throws {
        // Create a session
        let sessionId = try await sessionManager.createSession()
    ...
    func testFindElementsByQuery() async throws {
        // Create a session
        let sessionId = try await sessionManager.createSession()
    ...
    func testGetMostRecentSession() async throws {
        ...
        // Clean up
        try await sessionManager.cleanSession(sessionId: session1)
        try await sessionManager.cleanSession(sessionId: session2)
    }
