...
struct MCPToolRegistryExternalTests {
    ...
    func registerNativeTools() async {
        let registry = MCPToolRegistry()
    ...
    func testSetClientManager() async {
        let registry = MCPToolRegistry()
        let clientManager = TachikomaMCPClientManager()
    ...
    func registerExternalToolsEmpty() async {
        let registry = MCPToolRegistry()
        let clientManager = TachikomaMCPClientManager()
    ...
    func testGetToolsBySource() async {
        ...
        let nativeNames = categorized.native.map(\.name).sorted()
        #expect(nativeNames == ["native1", "native2"])
    }

    @Test("Get tool with prefix - native tools")
    func getToolWithPrefixNative() async {
        let registry = MCPToolRegistry()
    ...
    func getAllToolsCombined() async {
        let registry = MCPToolRegistry()
        ...
        let toolNames = allTools.map(\.name).sorted()
        ...
        #expect(externalTools.isEmpty)
    }

    @Test("Tool combined lookup")
    func toolCombinedLookup() async {
        let registry = MCPToolRegistry()
    ...
    func testAllToolInfos() async {
        ...
        let infoNames = toolInfos.map(\.name).sorted()
    ...
    func refreshExternalToolsWithClientManager() async throws {
        let registry = MCPToolRegistry()
        let clientManager = TachikomaMCPClientManager()
...
struct MCPToolRegistryMockExternalTests {
    ...
    func mixedToolRegistration() async {
        let registry = MCPToolRegistry()
    ...
    func toolLookupPrecedence() async {
        let registry = MCPToolRegistry()
