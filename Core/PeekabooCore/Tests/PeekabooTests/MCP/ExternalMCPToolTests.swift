...
struct CategorizedToolsTests {
    ...
    func testCategorizedToolsProperties() async {
        ...
        #expect(categorized.tools(from: "nonexistent").count == 0)
...
struct ToolOrganizerTests {
    ...
    func testToolFiltering() async {
        ...
        #expect(nativeFiltered.external.count == 0)
        ...
        #expect(mcpFiltered.native.count == 0)
        ...
        #expect(serverFiltered.native.count == 0)
    ...
    func testToolSorting() async {
        ...
        let sortedNativeNames = sortedTools.native.map { $0.name }
        #expect(sortedNativeNames == ["alpha", "beta", "zebra"])
        
        // Check external tools are sorted
        let sortedExternalNames = sortedTools.external["server1"]?.map { $0.name } ?? []
