    ...
    var body: some View {
        ...
        let _ = {
            if element.elementID.hasPrefix("B") || element.elementID.hasPrefix("C") || element.elementID.hasPrefix("Peekaboo") {
                Self.logger.debug("Element \(element.elementID) (\(element.displayName)): frame = \(element.frame.debugDescription)")
                Self.logger.debug("  App: \(element.appBundleID)")
            }
        }()
        ...
        let displayFrame = element.frame
