...
- `peekaboo click --on/--id`, `move --on/--id`, `drag --from/--to`, and `swipe --from/--to` now refresh the implicit observation snapshot once when a cached element ID is missing, avoiding stale latest-snapshot timeouts without overriding explicit `--snapshot`.
