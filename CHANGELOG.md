...
- `peekaboo click --on/--id` now refreshes the implicit observation snapshot once when the cached element ID is missing, avoiding stale latest-snapshot timeouts without overriding explicit `--snapshot`.
