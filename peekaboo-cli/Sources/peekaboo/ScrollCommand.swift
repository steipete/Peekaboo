...
struct ScrollCommand: AsyncParsableCommand {
    ...
    private func performScroll(direction: ScrollDirection,
                             amount: Int,
                             at location: CGPoint,
                             delayMs: Int,
    ...
    private func getScrollDeltas(for direction: ScrollDirection) -> (deltaX: Int, deltaY: Int) {
        switch direction {
        case .up:
            return (0, 5)  // Positive Y scrolls up
        case .down:
            return (0, -5) // Negative Y scrolls down
        case .left:
            return (5, 0)  // Positive X scrolls left
        case .right:
            return (-5, 0) // Negative X scrolls right
        }
    }
}
