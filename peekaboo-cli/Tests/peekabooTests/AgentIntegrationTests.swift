...
struct AgentIntegrationTests {
    ...
    @Test("Agent can execute simple TextEdit task", .enabled(if: ProcessInfo.processInfo.environment["RUN_AGENT_TESTS"] == "true"))
    func testAgentTextEditTask() async throws {
    ...
    @Test("Agent handles window automation", .enabled(if: ProcessInfo.processInfo.environment["RUN_AGENT_TESTS"] == "true"))
    func testAgentWindowAutomation() async throws {
        guard ProcessInfo.processInfo.environment["OPENAI_API_KEY"] != nil else {
            throw TestError.missingAPIKey
        }
        
        let outputFile = FileManager.default.temporaryDirectory.appendingPathComponent("agent-window-test-\(UUID()).json")
    ...
    func testDirectPeekabooInvocation() async throws {
        guard ProcessInfo.processInfo.environment["OPENAI_API_KEY"] != nil else {
            throw TestError.missingAPIKey
        }
        
        let outputFile = FileManager.default.temporaryDirectory.appendingPathComponent("direct-invocation-\(UUID()).json")
    ...
    func testAgentMaxSteps() async throws {
        guard ProcessInfo.processInfo.environment["OPENAI_API_KEY"] != nil else {
            throw TestError.missingAPIKey
        }
