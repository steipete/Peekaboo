...
@Suite("ScrollCommandV2 Tests", .serialized)
struct ScrollCommandV2Tests {
    @Test("ScrollV2 command parses direction", arguments: [
        "up", "down", "left", "right",
    ])
    func parseDirection(direction: String) throws {
        let command = try ScrollCommandV2.parse(["--direction", direction])
    ...
    @Test("ScrollV2 command parses all options")
    func parseAllOptions() throws {
        let command = try ScrollCommandV2.parse([
            "--direction", "down",
            "--amount", "5",
            "--on", "G1",
            "--session", "test-123",
    ...
    @Test("ScrollV2 command requires direction")
    func requiresDirection() {
        #expect(throws: Error.self) {
            _ = try ScrollCommandV2.parse([])
        }
    }

    @Test("ScrollV2 validates direction values", arguments: [
        ("up", true),
        ("down", true),
        ("left", true),
        ("right", true),
    ...
    @Test("ScrollV2 result structure")
    ...
    @Test("ScrollV2 smooth scrolling increases tick count")
    func smoothScrolling() throws {
        let normalCommand = try ScrollCommandV2.parse(["--direction", "down", "--amount", "3"])
        let smoothCommand = try ScrollCommandV2.parse(["--direction", "down", "--amount", "3", "--smooth"])
    ...
    @Test("ScrollV2 uses PeekabooCore services")
    func usesCoreServices() {
        // This test verifies that ScrollCommandV2 uses PeekabooServices
