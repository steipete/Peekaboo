...
struct AgentShellCommandTests {
    ...
    func shellFunctionExists() {
        ...
        let shellTool = OpenAIAgent.makePeekabooTool("shell", "Execute shell commands (use for opening URLs with 'open', running CLI tools, etc)")
        
        #expect(shellTool.type == "function")
        #expect(shellTool.function.name == "peekaboo_shell")
        #expect(shellTool.function.description == "Execute shell commands (use for opening URLs with 'open', running CLI tools, etc)")
        ...
        #expect(commandParam?["description"] as? String == "Shell command to execute (e.g., 'open https://google.com', 'ls -la', 'echo Hello')")
