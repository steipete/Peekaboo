...
struct TypeCommandTests {
    ...
    func typeWithText() async throws {
        let cli = CLI()
        let result = try await cli.run(args: ["type", "Hello World", "--json-output"])
    ...
    func typeWithSpecialKeys() async throws {
        let cli = CLI()
        let result = try await cli.run(args: ["type", "--tab", "2", "--return", "--json-output"])
    ...
    func typeWithClear() async throws {
        let cli = CLI()
        let result = try await cli.run(args: ["type", "New Text", "--clear", "--json-output"])
    ...
    func typeWithCustomDelay() async throws {
        let cli = CLI()
        let result = try await cli.run(args: ["type", "Fast", "--delay", "0", "--json-output"])
    ...
    func typeWithNoInput() async throws {
        let cli = CLI()
        let result = try await cli.run(args: ["type", "--json-output"])
