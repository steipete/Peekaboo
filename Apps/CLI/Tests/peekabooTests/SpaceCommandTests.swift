...
struct SpaceCommandTests {
    ...
    func spaceSwitchRequiresTo() async throws {
        do {
            _ = try await runPeekabooCommand(["space", "switch"])
    ...
    func spaceMoveWindowRequiresApp() async throws {
        do {
            _ = try await runPeekabooCommand(["space", "move-window", "--to", "2"])
    ...
    func spaceMoveWindowRequiresDestination() async throws {
        do {
            _ = try await runPeekabooCommand(["space", "move-window", "--app", "Finder"])
