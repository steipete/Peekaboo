...
struct RunCommandV2: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "run-v2",
        abstract: "Execute a Peekaboo automation script (V2 - uses services)",
        discussion: """
            The 'run-v2' command executes a batch script containing multiple
            ...
            EXAMPLES:
              peekaboo run-v2 login-flow.peekaboo.json
              peekaboo run-v2 test-suite.json --output results.json
              peekaboo run-v2 automation.json --no-fail-fast
    ...
    mutating func run() async throws {
        ...
        do {
            ...
            // Prepare output
            let output = ScriptExecutionResultV2(
...
// MARK: - V2 Output Model

struct ScriptExecutionResultV2: Codable {
    let success: Bool
