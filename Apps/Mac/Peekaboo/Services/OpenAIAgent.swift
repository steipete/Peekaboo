...
actor OpenAIAgent {
    ...
    func executeTask(_ task: String, dryRun: Bool = false) async throws -> PeekabooCore.AgentResult {
        self.logger.info("Starting task: \(task)")
        
        // Emit started event
        await self.emitEvent(.started(task: task))
        ...
        while stepCount < self.maxSteps {
            ...
            case .completed:
                ...
                await self.emitEvent(.completed(summary: summary))

                return PeekabooCore.AgentResult(
                    steps: outputs.enumerated().map { index, output in
                        PeekabooCore.AgentResult.Step(
