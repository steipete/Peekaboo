...
final class RealtimeVoiceService {
    ...
    func startSession() async throws {
        self.logger.info("Starting realtime voice session")
        ...
        self.logger.debug("Registered \(tools.count) tools for realtime session")
        ...
        do {
            ...
            self.logger.info("Realtime voice session started successfully")
        } catch {
            self.error = error
            self.logger.error("Failed to start realtime session: \(error)")
            throw error
        }
    }
    ...
    func endSession() async {
        self.logger.info("Ending realtime voice session")

        // Cancel monitoring tasks
        for task in self.monitoringTasks {
            task.cancel()
        }
           ...
           let session = sessionStore.sessions.first(where: { $0.id == sessionId })
        {
            ...
            if !self.currentTranscript.isEmpty {
                self.sessionStore.addMessage(
                    ConversationMessage(role: .assistant, content: self.currentTranscript),
                    to: session)
            }
        }

        self.currentSessionId = nil
        self.logger.info("Realtime voice session ended")
    }
    ...
    func updateVoice(_ voice: RealtimeVoice) {
        self.selectedVoice = voice
        self.settings.realtimeVoice = voice.rawValue
    ...
    private func startMonitoring() async {
        ...
        self.monitoringTasks.insert(transcriptTask)
        ...
        self.monitoringTasks.insert(stateTask)
        ...
        self.monitoringTasks.insert(audioTask)
    }
}
...
enum RealtimeError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .notConnected:
            "Not connected to Realtime API"
        case .apiKeyMissing:
            "OpenAI API key is required for Realtime API"
