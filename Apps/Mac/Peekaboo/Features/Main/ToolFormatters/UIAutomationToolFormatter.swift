...
struct UIAutomationToolFormatter: MacToolFormatterProtocol {
    ...
    func formatSummary(toolName: String, arguments: [String: Any]) -> String? {
        switch toolName {
        case "click":
            self.formatClickSummary(arguments)
        case "type":
            self.formatTypeSummary(arguments)
        case "scroll":
            self.formatScrollSummary(arguments)
        case "hotkey":
            self.formatHotkeySummary(arguments)
        case "press":
            self.formatPressSummary(arguments)
        case "dialog_click":
            self.formatDialogClickSummary(arguments)
        case "dialog_input":
            self.formatDialogInputSummary(arguments)
        case "dock_click":
            self.formatDockClickSummary(arguments)
        default:
            nil
        }
    }
